...
> **Generated**: 2026-09-13 21:19:28 UTC  
...
| k-NN | Supervised Machine Learning | 40 cases/fold | 63.75 ± 7.93 | 69.73 ± 6.99 | [66.1, 73.7] | 0.1994 | **0.9968** | -3.7% [-9.1, 1.6] | -0.06 |
...
*Note on Protocol Differences & ML Recall Artifact: Aggregate exact match is dominated by the 18/23 negative control cases (78.3% of the benchmark), on which returning `No_Diagnosis` is correct. Positive-case recall over the 5 in-scope disease cases is reported separately and is the diagnostically meaningful column. Importantly, the near-zero positive recall for supervised ML in this table (0.00% for 4 models, 10.00% for Decision Tree) is a protocol artifact of 5×2-fold cross-validation strictly within the 23 cases of the `eval` split: each training fold receives only 11 cases (9 negative controls and at most 2 positive cases across 4 classes), meaning entire disease classes (e.g. Rice Tungro Virus, Rice Grassy Stunt) have zero training examples. Under 82% negative prevalence, supervised models predictably converge to predicting `No_Diagnosis`. This 0/5 figure must NOT be cited as evidence of inherent ML weakness. In contrast, when trained on the separate `dev` split (n=16, containing 7 positive cases) and evaluated on `eval` as in the Cold-Start Learning Curve (Pool B, Section 3 of `results/learning_curve.md`), supervised models correctly identify present classes and reach 40.0% positive recall (2/5). Table 2 and the learning curve answer different questions and must not be conflated. Risk Difference (\Delta Acc) is reported with paired Wald 95% CI. Cohen's g is bounded on $[-0.50, +0.50]$ and saturates; read the Risk Difference for magnitude.*
...
2. **Supervised 0% Recall Is a Protocol Artifact, Not Model Weakness**: In Table 2's 5×2-fold CV, ML classifiers train on only 11 cases per fold with $\le 2$ positive instances across 4 classes. Their 0.00% positive recall (10.00% for Decision Tree) is an artifact of training-fold class sparsity and negative class imbalance (82% negative controls), which forces prediction of `No_Diagnosis`. When trained on the 16 cases of the `dev` partition (Pool B of the learning-curve experiment), ML models achieve 40.0% positive recall on `eval`.
