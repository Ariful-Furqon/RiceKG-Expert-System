...
Generated by the scripts in `analysis/` and `baselines/`; each section is rewritten by the script that produces it (see `docs/RESULTS_INDEX.md` for the command behind every figure). Structured data for each section is in the matching `results/*.json`.
