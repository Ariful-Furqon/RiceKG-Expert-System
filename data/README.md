...
2. **Stage B (Vocabulary Mapping & Annotator Status)**:
   - `annotator_id` is set to `"unassigned"` pending formal human agronomist multi-rater trial. The multi-rater instrument is ready (`docs/ANNOTATION_PROTOCOL.md`); its de-identified per-rater answers are written to `data/annotations_*.csv` and `data/definition_review.csv`, which are kept locally (git-ignored) because raters were told answers would be reported only in aggregate; the agreed encoding is `data/symptom_encoding_consensus.csv`.
