...
- **Multi-Tier SWRL Rules**: 18 deterministic forward-chaining rules over 61 observation terms (6 Tier-1 canonical rules; 12 Tier-2 rules, six composite and six single diagnostic-sign rules), linking phenotypic signs to graded diagnoses. Every rule cites its literature source in `ricekg/model.py` (`RULE_REGISTRY`).
...
Every figure quoted in this README is regenerated by a script and traceable
...
| Ontology competency questions | `make competency` | [`results/REPORT.md#competency-questions`](results/REPORT.md#competency-questions) |
...
| Document | Covers |
|---|---|
| [`results/REPORT.md#competency-questions`](results/REPORT.md#competency-questions) | 16 competency questions (Grüninger & Fox) with the SPARQL that answers each. |
| [`ontology/term_definitions.csv`](ontology/term_definitions.csv) | Operational definition, labels and source of every ontology term. |
| [`results/REPORT.md`](results/REPORT.md) | Every result, one section per analysis, with the command that produces each figure. |

---

## Evaluation Results

The evaluation has four parts: verification of
...
serves as regression input to the reasoner-equivalence check.
...
- **Explainability vs Accuracy Framing**: RiceKG's contribution is zero-shot cold start, deductive auditability, and graded clinical confidence without training data, within the limits stated above.
